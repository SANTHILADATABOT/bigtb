...
class Menu
{
    ...
    public static function admin_menu()
    {
        ...
        $mytask_text = sprintf('My Tasks %s',
            '<span class="awaiting-mod count-1"><span class="pending-count">' . $active_task . '</span></span>');
        $submenu[$slug][] = [$mytask_text, self::$capability, "admin.php?page={$slug}#/my-tasks"];

        if (pm_user_can_access(pm_manager_cap_slug())) {
            $submenu[$slug][] = [
                'Categories',
                self::$capability, "admin.php?page={$slug}#/categories"
            ];
        }
        ...
        if (pm_user_can_access(pm_manager_cap_slug())) {
            $submenu[$slug]['importtools'] = ['Tools', self::$capability, "admin.php?page={$slug}#/importtools"];
        }

        do_action('pm_menu_after_load_scripts', $home);
    }

    public static function pm_svg():string
    {
